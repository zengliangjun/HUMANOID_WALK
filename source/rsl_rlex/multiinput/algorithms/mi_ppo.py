...
from rsl_rlex.multiinput.modules import mi_modules
from rsl_rlex.multiinput.storage import mi_storage
...
class MIPPO(PPO):
    ...
    storage: mi_storage.RolloutStorage# type: ignore
    transition : mi_storage.RolloutStorage.Transition

    def __init__(self, actor_critic, alg_cfg: dict, device="cpu"):

        self.entropy_ranges = alg_cfg["entropy_ranges"] if hasattr(alg_cfg, "entropy_ranges") else (1.5, 10)
        self.entropy_coef_factor = alg_cfg["entropy_coef_factor"] if hasattr(alg_cfg, "entropy_coef_factor") else 1.05
        self.entropy_coef_scale = alg_cfg["entropy_coef_scale"] if hasattr(alg_cfg, "entropy_coef_scale") else 10

        self.entropy_smoothing = alg_cfg["entropy_smoothing"] if hasattr(alg_cfg, "entropy_smoothing") else 0.9 # 熵值平滑系数
        self.use_adaptive_ranges = alg_cfg["use_adaptive_ranges"] if hasattr(alg_cfg, "use_adaptive_ranges") else True # 是否启用自适应熵范围
        self.reward_scale = alg_cfg["reward_scale"] if hasattr(alg_cfg, "reward_scale") else 1.0 # 回报缩放因子

        alg_cfg.pop("entropy_ranges", None)
        alg_cfg.pop("entropy_coef_factor", None)
        alg_cfg.pop("entropy_coef_scale", None)
        alg_cfg.pop("entropy_smoothing", None)
        alg_cfg.pop("use_adaptive_ranges", None)
        alg_cfg.pop("reward_scale", None)

        super(MIPPO, self).__init__(actor_critic, device=device, **alg_cfg)
        self.entropy_coef_org = self.entropy_coef


    def init_storage(self, num_envs, num_transitions_per_env, action_shape, obs):
        self.storage = mi_storage.RolloutStorage(
            num_envs,
        ...
        self.transition = mi_storage.RolloutStorage.Transition()
        ...
        for (
            ...
            if self.symmetry:
                ...
                for name, value in obs_batch.items():
                    clones[name] = value.detach().clone()

                mean_actions_batch = self.actor_critic.act_inference(clones)
